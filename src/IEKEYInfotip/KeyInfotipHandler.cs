...
namespace IEAREInfotip
{
    /// <summary>
    /// This handler shows configurable information from an Infinity Engine KEY file
    /// </summary>
    [ComVisible(true)]
    [COMServerAssociation(AssociationType.ClassOfExtension, ".key")]
    public class KeyInfotipHandler : SharpInfoTipHandler
    {
        ...
        public string ProcessFile(string filename, string tlkLocationData)
        {
            ...
            using (PowerShell PowerShellInst = PowerShell.Create())
            {
                ...
                {
                    var path = Path.Combine(Environment.GetEnvironmentVariable("ieshellex", EnvironmentVariableTarget.Machine), "config", "key.ps1");
