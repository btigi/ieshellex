...
namespace IEITMInfotip
{
    /// <summary>
    /// This handler shows configurable information from an Infinity Engine VVC file
    /// </summary>
    [ComVisible(true)]
    [COMServerAssociation(AssociationType.ClassOfExtension, ".vvc")]
    public class VvcInfotipHandler : SharpInfoTipHandler
    {
        ...
        public string ProcessFile(string filename, string tlkLocationData)
        {
            ...
            using (PowerShell PowerShellInst = PowerShell.Create())
            {
                ...
                {
                    var path = Path.Combine(Environment.GetEnvironmentVariable("ieshellex", EnvironmentVariableTarget.Machine), "config", "vvc.ps1");
