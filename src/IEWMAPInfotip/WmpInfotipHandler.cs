...
namespace IEWMPInfotip
{
    /// <summary>
    /// This handler shows configurable information from an Infinity Engine WMP file
    /// </summary>
    [ComVisible(true)]
    [COMServerAssociation(AssociationType.ClassOfExtension, ".wmp")]
    public class WmpInfotipHandler : SharpInfoTipHandler
    {
        ...
        public string ProcessFile(string filename, string tlkLocationData)
        {
            ...
            using (PowerShell PowerShellInst = PowerShell.Create())
            {
                ...
                {
                    var path = Path.Combine(Environment.GetEnvironmentVariable("ieshellex", EnvironmentVariableTarget.Machine), "config", "wmp.ps1");
