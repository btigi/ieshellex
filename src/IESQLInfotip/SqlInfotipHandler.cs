...
namespace IESQLInfotip
{
    /// <summary>
    /// This handler shows configurable information from an Infinity Engine SQL file
    /// </summary>
    [ComVisible(true)]
    [COMServerAssociation(AssociationType.ClassOfExtension, ".sql")]
    public class SqlInfotipHandler : SharpInfoTipHandler
    {
        ...
        public string ProcessFile(string filename, string tlkLocationData)
        {
            ...
            using (PowerShell PowerShellInst = PowerShell.Create())
            {
                ...
                {
                    var path = Path.Combine(Environment.GetEnvironmentVariable("ieshellex", EnvironmentVariableTarget.Machine), "config", "sql.ps1");
