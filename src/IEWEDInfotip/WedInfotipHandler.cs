...
namespace IEWMAPInfotip
{
    /// <summary>
    /// This handler shows configurable information from an Infinity Engine WED file
    /// </summary>
    [ComVisible(true)]
    [COMServerAssociation(AssociationType.ClassOfExtension, ".wed")]
    public class WedInfotipHandler : SharpInfoTipHandler
    {
        ...
        public string ProcessFile(string filename, string tlkLocationData)
        {
            ...
            using (PowerShell PowerShellInst = PowerShell.Create())
            {
                ...
                {
                    var path = Path.Combine(Environment.GetEnvironmentVariable("ieshellex", EnvironmentVariableTarget.Machine), "config", "wed.ps1");
                    if (!string.IsNullOrEmpty(path))
                    {
                        PowerShellInst.AddScript(File.ReadAllText(path), false);
                        PowerShellInst.Invoke();
                        PowerShellInst.AddCommand("GetData");
                        PowerShellInst.AddParameter("fs", fs);
                    }
